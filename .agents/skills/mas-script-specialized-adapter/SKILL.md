...
6. 就地本地编写并运行最小专项测试验证改动，测试文件不提交（判据见根目录 `AGENTS.md`「分支与 PR」）；**测试缺口写进结果，不编造验证结果**。
...
改动现场就地写最小专项验证用例并跑一遍，跑完不提交（仓库只留跨功能通用的公共与纯逻辑测试，见根目录 `AGENTS.md`「分支与 PR」）。没有可复用测试时**不为填目录新增测试文件**，在结果中说明测试缺口。仅在实际涉及前端时才从 `frontend` 跑前端测试；跨模块契约或用户明确要求时才扩大范围。文档修改至少用 `rg` 确认不存在相互冲突的旧规则。
